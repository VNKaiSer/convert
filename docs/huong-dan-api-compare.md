...
Mỗi **hàng mapping** gồm **Array** + **Body field**. Bấm **+ Thêm cặp array → body field** để thêm hàng (không giới hạn cứng 2 hàng):

| Hàng | Bắt buộc | Vai trò |
| ---- | -------- | ------- |
| **Hàng 1** — Array (vòng lặp) + Body field | Có | Quyết định **số lần gọi API**; inject `array[i]` (vd. `pcbRaw` ↔ `{{pcbRaw}}`) |
| **Hàng 2, 3, …** | Tùy chọn | Inject array khác cùng index `i` nếu còn phần tử (vd. `icoreRaw` ↔ `{{icoreRaw}}`) |
...
**Chỉ 1 array:** giữ 1 hàng mapping; xoá hàng thừa bằng nút thùng rác.
...
{
  "mappings": [
    { "sourceVariable": "pcbRaws", "injectVariable": "pcbRaw" },
    { "sourceVariable": "icores", "injectVariable": "icoreRaw" }
  ]
}
```

- `mappings[0]`: cặp chính (vòng lặp)
- `mappings[1+]`: các cặp bổ sung (cùng index, tuỳ chọn)
